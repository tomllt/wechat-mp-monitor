...
2. **自定义私有 Worker**：部署自己的 Cloudflare Worker 反向代理，配置到环境变量
...
| `feature/worker-proxy-concurrency` | ✅ 功能完整 | 并发下载 + Worker 代理池 + 健康检查 + 多级导出 |
...
1. 配置可用的自定义私有 Worker（内置 96 个 Worker 全部 403 不可用）
2. 执行 `./bin/wechat-mp-monitor download pending --no-proxy` 完成剩余文章下载（住宅 IP 环境下可直接直连下载）
3. 合并功能分支到 main
4. 推送最终代码到 GitHub
